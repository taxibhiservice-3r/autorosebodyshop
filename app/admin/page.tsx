...
const IS_CONTACT = (b: Booking) => b.service === "[Contact Form]";
...
const FILTERS: { key: "all" | BookingStatus | "contact" | "quote"; label: string }[] = [
  { key: "all",       label: "All" },
  { key: "quote",     label: "Estimates" },
...
type FilterKey = "all" | BookingStatus | "contact" | "quote";
...
function buildCopyText(b: Booking): string {
  const isContact = IS_CONTACT(b);
  const vehicle = isContact
    ? b.vehicle_make
    : [b.vehicle_year, b.vehicle_make, b.vehicle_model].filter(Boolean).join(" ");

  const lines = [
    `${isContact ? "CONTACT MESSAGE" : "ESTIMATE REQUEST"} — ${formatDate(b.created_at)}`,
    ...
    vehicle ? `\nVEHICLE\n${vehicle}` : null,
    !isContact && b.service ? `\nSERVICE\n${b.service}` : null,
    !isContact && b.insurance_claim ? `Insurance: ${b.insurance_claim}` : null,
    !isContact && b.preferred_date ? `Preferred drop-off: ${formatPreferredDate(b.preferred_date)}` : null,
    b.description ? `\n${isContact ? "MESSAGE" : "DAMAGE DESCRIPTION"}\n${b.description}` : null,
  ].filter((l) => l !== null);
  return lines.join("\n");
}
...
export default function AdminPage() {
  ...
  // ── Counts ────────────────────────────────────────────────────────────────
  const quoteCount   = bookings.filter((b) => !IS_CONTACT(b)).length;
  const contactCount = bookings.filter((b) => IS_CONTACT(b)).length;
  const newCount     = bookings.filter((b) => b.status === "new").length;
  const todayStr     = new Date().toDateString();
  const todayCount   = bookings.filter((b) => new Date(b.created_at).toDateString() === todayStr).length;

  function filterCount(key: FilterKey) {
    if (key === "all")     return bookings.length;
    if (key === "quote")   return quoteCount;
  ...
  const filtered = bookings.filter((b) => {
    ...
    if (filter === "quote")   return !IS_CONTACT(b);
    if (filter === "contact") return IS_CONTACT(b);
      ...
      <header className="bg-[#1A1B1E] border-b border-[#2E3035] sticky top-0 z-20">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="hidden sm:block text-[#9CA3AF] text-sm">— Inbox</span>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: "Estimates", value: quoteCount },
        ...
        {!loading && !fetchError && filtered.length === 0 && (
          <div className="text-center py-16 text-[#9CA3AF]">No submissions yet.</div>
        )}
        ...
        <div className="space-y-4">
          {filtered.map((booking) => {
            ...
            const isContact = IS_CONTACT(booking);
            const vehicle = isContact
            ...
            return (
              <div key={booking.id} className="bg-[#1A1B1E] border border-[#2E3035] rounded-2xl overflow-hidden">
                {/* Card header */}
                ...
                <div className="px-5 py-4 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-x-6 gap-y-1">
                    ...
                    {!isContact && booking.insurance_claim && (
                      <div>
                    ...
                    {!isContact && booking.preferred_date && (
                      <div>
                  ...
                  {booking.description && (
                    <div>
                      <span className="text-[#9CA3AF] text-xs uppercase tracking-wide">
                        {isContact ? "Message" : "Damage Description"}
                      </span>
                      <p className="text-[#9CA3AF] text-sm leading-relaxed border-l-2 border-[#2E3035] pl-3 mt-1">
                        {booking.description}
                      </p>
                    </div>
                  )}
                ...
                <div className="px-5 py-3 border-t border-[#2E3035] flex items-center gap-3">
                  ...
                  >
                    {isCopied ? (
                      ...
                      <><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>Copy Details</>
                    )}
                  </button>

                  {isDeleteConfirming ? (
                    <div className="flex items-center gap-2 ml-auto">
                      <span className="text-[#9CA3AF] text-xs">Delete this?</span>
                      <button
